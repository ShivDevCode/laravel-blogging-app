...
<section class="module-small">
    <div class="container">
      <div class="row">
        <div class="col-sm-8">
          <div class="post">
            <div class="post-thumbnail"><img style="width: 100%" src="../storage/cover_images/{{$post->cover_image}}" alt="Blog Featured Image"/></div>
            <div class="post-header font-alt">
              <h1 class="post-title">{{ $post->title }} - <span style="color: grey">{{ $post->likes->count() }}{{ Str::plural('like', $post->likes->count()) }}</span></h1>
              <div class="post-meta">By&nbsp;<a href="#">{{ $post->user->username }}</a>| {{ $post->created_at->diffForHumans() }} | <a href="#">{{ $post->category->name}}</a>
              </div>
            </div>
            <div class="row">
              @auth
              @if (!$post->likedBy(auth()->user()))
              <form action="{{ route('posts.likes',$post) }}" method="post" class="col-xs-3 col-lg-2">
                @csrf
                <button class="btn btn-info btn-round btn-xs" type="submit"><div class="col-md-4 col-sm-6 col-lg-3"><span></span>&nbsp;<i class="fa fa-fw"></i></div></button>
              </form>
              @else
              <form action="{{ route('posts.likes',$post) }}" method="post" class="col-xs-3 col-lg-2">
                @csrf
                @method('DELETE')
                <button class="btn btn-danger btn-round btn-xs" type="submit"><div class="col-md-4 col-sm-6 col-lg-3"><i class="fa fa-fw"></i></div></button>
              </form>
              @endif
              @endauth
            </div>
            ...
            <ul class="widget-posts">
              ...
              <li class="clearfix">
                <div class="widget-posts-image"><a href="/blog/{{$post->id}}"><img
                            src="../storage/cover_images/{{$post->cover_image}}" alt="Post Thumbnail" width="80px" height="70px"/></a>
                </div>
                <div class="widget-posts-body">
                    <div class="widget-posts-title"><a href="/blog/{{$post->id}}">{{$post->title}}</a></div>
                    <div class="widget-posts-meta">{{ $post->created_at->diffForHumans() }}</div>
                </div>
            </li>
              @endif
              @endforeach
